...
describe('integration: SUPERHERO', () => {
  it('should return superhero details', async () => {
    const { server, superheroApi } = constructTestServer();
    ...
    jest.spyOn(superheroApi as any, 'get').mockImplementation(() => mockRes);

    const result = await server.executeOperation({
      query: SEARCH_SUPER_HERO,
      variables: {
        name: 'batman',
      },
    });
